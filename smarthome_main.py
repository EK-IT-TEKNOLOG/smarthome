...
#
import math, sys, uselect              # Needed for USB port use
...
def mac_addr_to_bstring(addr):
    if len(addr) == 17:
        ...
        if len(parts) == 6:
            ...
            mac_addr_bstring = bytes([ba[0], ba[1], ba[2], ba[3], ba[4], ba[5]])
            return mac_addr_bstring
...
# Load the user name
user_name_ee = eeprom.read_string(EEPROM_USER_NAME)
if user_name_ee[0] == b'\xc3':
    user_name = user_name[0:20]
    eeprom.write_string(EEPROM_USER_NAME, user_name)
else:
    user_name = user_name_ee
...
while True:
    ...
    host, msg = en.esp_now_receive_message() # Check to see if there is a message and return it and the transmitter MAC address is any
    if msg:
